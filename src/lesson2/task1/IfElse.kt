...
fun ageDescription(age: Int): String {
    if (age in 5..20 || age % 10 in 5..9 || age == 111) {
        return "$age лет"
    }
    if (age == 1 || age % 10 == 1) {
        return "$age год"
    }
    if (age in 2..4 || age % 10 in 2..4) {
        return "$age года"
    }
...
fun timeForHalfWay(
    ...
    return when {
        halfWay <= s1 -> {
            halfWay / v1 // весь первый отрезок меньше или равен полпути, делим на скорость первого отрезка
        }
        halfWay <= s1 + s2 -> {
            t1 + (halfWay - s1) / v2 // добавляем время первого отрезка и вычитаем из полпути первый отрезок
        }
        else -> {
            t1 + t2 + ((halfWay - s1 - s2) / v3) // добавляем время первого и второго отрезка и вычитаем из полпути первый и второй отрезок
        }
